...
\item Peers which receive a Ping message must respond with a Pong message (if the peer supports Pong message).
\item If no Ping or Pong message is received from a peer for some time, a node may assume that there is some problem with this peer.
...
\item A node (if Pong message is supported) receiving this kind of Ping message SHOULD respond with a Pong message which contains maximum five entries of its neighbour peers, excluding the one which sent the Ping message.
...
Correspondingly, there are also two types of Pong messages.
Since a Pong message responds to a Ping message, the original sender IP, sender port and message ID are identical to the Ping message.
\begin{itemize}
\item A Pong message responding to an availability test.
This Pong message carries no payload.
\item A Pong message responding to a network probe.
This Pong message contains the neighbour information of the peer in the body.
See the description for the corresponding Ping message for details.
\end{itemize}
...
It SHOULD also forward this message to its neighbours (if TTL is larger than 0) no mater the query is a hit or not in its local shared files.

The length of search criteria varies, and the actual length is determined by the Payload length field in the message header.
Nodes MUST NOT change the message ID of a Query Message while forwarding.

A node SHOULD keep a record of recently received Query Messages.
If a node receives a Query Message whose ID is identical to a recent query, the message SHOULD be discarded.
...
Your application should be able to connect to any given IP and port combination at the demo.
In other words, the peer list shall not be hard coded (or at least in a way that is easy to change).
