...
\end{itemize}
In order to test and observe the behavior of P2P nodes, an internal P2P network is constructed. This network is Gnutella-alike style (and simplified version), without a central facility. Each node uses a piece of bootstrap information to join the network. In order to communicate with other nodes, your application should fulfill the requirements of the protocol. For the details of the protocol, please refer to the next section. Grading is based on numbers of features your application provides, the final report and the demo. Please read the grading section for more information.
...
A new node sends Join Request to a node already residing in the network for the permission of entering the network. This is the so-called bootstrap process. The information about the bootstrap server will given soon so you can start testing your code as soon as possible.
...
A related I/O API you might be interested in is called “select”, which is both supported by C and Java.
...
The common debugging tool for C is GDB, which enables you to set break point in you application and run the program step by step. There is also similar debugger in Java, and can be easily launched in IDE like Eclipse or Netbeans. Sometimes, printing out the information also helps you solve the problem.
...
\end{itemize}
Each team will be assigned a repository from version control system (Git), for source code management and submission. Please check \emph{http://git.nikusla.hut.fi}. Please check Git documentation if you haven't used it before (\emph{http://git-scm.com/doc}).
\section{Demo}
...
\subsection*{Advanced (80 points)}
\begin{table}[htdp]
\begin{center}
\begin{tabular}{|p{11cm}|p{2cm}|}
\hline
Respond Type A Ping message with Pong. Tell you neighbor that you are still alive. & 10 points \\
\hline
Send Type A Ping message. Check if your neighbors are alive. & 10 Points \\
\hline
